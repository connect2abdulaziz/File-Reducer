	...
	return (
		<Downshift
			...
			{({
				...
				if (isOpen) {
					const filteredOrderedChoices = orderedChoices.filter(
						(item) =>
							!inputValue ||
							(orderedChoices.find(
								({ key }) =>
									key.toString() === selectedItem.toString()
							) &&
								orderedChoices.find(
									({ key }) =>
										key.toString() ===
					...
					dropdown = (
						<div
							...
							{filteredOrderedChoices.map((item, index) => (
								<Fragment key={index}>
									{item.group &&
										(index === 0 ||
											filteredOrderedChoices[index - 1]
												.group !==
												filteredOrderedChoices[index]
													.group) && (
											<div
